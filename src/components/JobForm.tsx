...
export default function JobForm() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isValid, isSubmitting } } = useForm<JobFormValues>({
    resolver: zodResolver(JobFormSchema),
    mode: 'onChange',
    defaultValues: jobToEdit
      ? { ...jobToEdit, tags: jobToEdit.tags.join(', '), postedAt: jobToEdit.postedAt.split('T')[0] }
      : {
          title: '',
          company: '',
          location: '',
  ...
  return (
    <div className="job-form-card mx-auto p-6 max-w-2xl shadow-lg rounded-lg bg-white">
      <h1 className="job-form-title mb-6">{jobToEdit ? 'Редактировать вакансию' : 'Создать вакансию'}</h1>
      <form onSubmit={handleSubmit(onSubmit)} className="job-form">
        ...
        <div className="job-form-row">
          <div className="job-form-field">
            ...
            <input type="number" {...register('salaryFrom', { valueAsNumber: true })} className="job-form-input" />
            {errors.salaryFrom && <p className="job-form-error">{errors.salaryFrom.message}</p>}
          </div>
          <div className="job-form-field">
            <label className="job-form-label">Зарплата до</label>
            <input type="number" {...register('salaryTo', { valueAsNumber: true })} className="job-form-input" />
        ...
        <div className="job-form-field">
          ...
          <input {...register('tags')} className="job-form-input" />
          {errors.tags && <p className="job-form-error">{errors.tags.message}</p>}
        </div>

        <div className="job-form-field">
          <label className="job-form-label">Дата публикации</label>
          <input type="date" {...register('postedAt')} className="job-form-input" />
          {errors.postedAt && <p className="job-form-error">{errors.postedAt.message}</p>}
        </div>

        <div className="job-form-field">
          <label className="job-form-label">Описание</label>
          <textarea {...register('description')} className="job-form-textarea" rows={5}></textarea>
